...
const Hero = () => {
  ...
  const stats = [
    ...
    { num: "2.4M", text: "Revenue Generated", color: "from-green-400 to-emerald-400" },
    ...
    <section id="hero-section" className="relative min-h-screen flex items-center justify-center overflow-hidden pt-25 pb-20">
      ...
      <div className="relative z-10 container mx-auto px-6 text-center">
        <div className="max-w-6xl mx-auto">
          ...
          <div className={`inline-flex items-center px-6 py-3 rounded-full backdrop-blur-xl bg-white/10 border border-white/20 mb-8 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          ...
          <div className="mb-16"
            ...
            <div className="flex flex-wrap items-center justify-center gap-8 opacity-60 hover:opacity-80 transition-opacity duration-300">
              ...
              {['TechFlow', 'InnovateLab', 'GrowthCorp', 'ScaleUp', 'FutureAI'].map((company, i) => (
                <div key={i} className="backdrop-blur-sm bg-white/5 border border-white/10 px-6 py-3 rounded-xl hover:bg-white/10 transition-all duration-300 hover:scale-105">
        ...
        <div className="relative max-w-6xl mx-auto"
          ...
          <div id="dashboard-preview" className="relative backdrop-blur-xl bg-gradient-to-br from-white/10 to-white/5 border border-white/20 rounded-3xl p-8 shadow-2xl">
            ...
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/10">
              ...
              <div className="flex items-center space-x-3">
                <div className="flex space-x-2">
                  <button onClick={closeBrowser} className="w-3.5 h-3.5 bg-red-500 rounded-full hover:bg-red-400 cursor-pointer" />
                  <button onClick={minimizeDashboard} className="w-3.5 h-3.5 bg-yellow-500 rounded-full hover:bg-yellow-400 cursor-pointer" />
              ...
              <div className="flex-1 max-w-md mx-4">
                <div className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 flex items-center">
                  ...
                  <span className="text-sm text-gray-300 font-mono">dashboard.admybrand.com</span>
                </div>
              </div>

              {/* Browser controls - basic */}
              <div className="flex items-center space-x-2">
                <button onClick={refreshDashboard} className="w-6 h-6 bg-white/5 border border-white/10 rounded-md flex items-center justify-center hover:bg-white/10 cursor-pointer">
                  <svg className={`w-3 h-3 text-gray-400 ${refreshing ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                  </svg>
                </button>
                <button onClick={() => setMenuOpen(!menuOpen)} className="w-6 h-6 bg-white/5 border border-white/10 rounded-md flex items-center justify-center hover:bg-white/10 cursor-pointer">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {stats.map((stat, index) => (
                <div key={index} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-all duration-300 hover:scale-105">
                  ...
                  <div className={`mt-3 h-1 bg-gradient-to-r ${stat.color} rounded-full opacity-60`} />
                </div>
              ))}
            </div>
